...
export default function AppLayout() {
  ...
  return (
    <div 
      className={`bg-[#F8F9FA] text-gray-900 font-sans h-screen w-full overflow-hidden flex relative transition-all duration-300 ${showNotchMargin ? 'pt-10' : 'pt-0'}`}
    >
      <div className="flex w-full h-full bg-[#F8F9FA] overflow-hidden">
        ...
        <div className="flex-1 flex flex-col h-full relative overflow-hidden">
          ...
          <nav className="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 px-6 py-3 flex justify-between items-center pb-safe shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
            ...
            <MobileNavLink to="/settings" icon="fa-gear" label="Settings" />
          </nav>
        </div>
      </div>
    </div>
